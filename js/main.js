...
async function cargarPrimeroLoLocal() {
  ...
  try { manualFeatures = await cargarManual(); }
  catch (error) { console.error('Error cargando datos manuales:', error); manualFeatures = FALLBACK_GEOJSON.features; }
  refrescarVista();
...
async function cargarLuegoOSM() {
  try {
    ...
    refrescarVista();
    loadingEl.textContent = `${manualFeatures.length} manual(es) + ${osmFeatures.length} OSM = ${datosOriginales.features.length} cancha(s) ✓`;
  } catch (error) {
    console.warn('Overpass no respondió a tiempo:', error);
    loadingEl.textContent = `${manualFeatures.length} cancha(s) verificada(s) ✓ (OpenStreetMap no respondió, usa "Recargar")`;
  }
}

async function recargarTodo() { osmFeatures = []; await cargarPrimeroLoLocal(); await cargarLuegoOSM(); }
